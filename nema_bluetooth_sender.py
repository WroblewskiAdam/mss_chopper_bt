...
class NmeaBluetoothSender:
    ...
    BAUD_RATE = 115200
    TARGET_BT_MAC = '2C:CF:67:A2:F3:ED'
    TARGET_BT_PORT = 4

    # --- Konfiguracja Klienta NTRIP ---
    # !!! WPROWADŹ SWOJE DANE LOGOWANIA DO SERWERA NTRIP PONIŻEJ !!!
    ...
    NTRIP_GGA_INTERVAL = 10  # Co ile sekund wysyłać pozycję GGA do serwera NTRIP
    ...
    def __init__(self):
        ...
        # Zmienne do obsługi NTRIP
        self.last_gga_for_ntrip = None
        self.last_gga_send_time = 0

        # Mechanizmy synchronizacji i zatrzymywania
    ...
    def connect_bluetooth(self):
        while not self.stop_event.is_set():
            try:
                ...
                # Zamknij stare gniazdo, jeśli istnieje
    ...
    def connect_ntrip(self):
        ...
        try:
            ...
            if b"ICY 200 OK" in response:
                ...
                print(f"Wysyłanie początkowego GGA do NTRIP: {self.last_gga_for_ntrip.strip()}")
                self.ntrip_sock.sendall(self.last_gga_for_ntrip.encode('ascii'))
                self.last_gga_send_time = time.time()
                return True
            else:
                print(f"Błąd autoryzacji NTRIP. Odpowiedź serwera: {response.decode('ascii', 'ignore')}")
    ...
    # --- Wątek: Odczyt z GPS i zarządzanie połączeniem NTRIP ---
    def read_serial_data_thread(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if msg_type.endswith("GGA") and len(parts) > 9:
                    with self.data_lock:
                    ...
                    # Zapisz ostatnią wiadomość GGA dla NTRIP
                    self.last_gga_for_ntrip = line + "\r\n"

                elif msg_type.endswith("VTG") and len(parts) > 7:
                    with self.data_lock:
                        speed_kph = float(parts[7]) if parts[7] else 0.0
                        self.latest_gps_data['speed'] = speed_kph * (1000.0 / 3600.0)
                        self.latest_gps_data['heading'] = float(parts[1]) if parts[1] else 0.0

                # --- Logika zarządzania połączeniem NTRIP ---
                if self.last_gga_for_ntrip:
                    # Jeśli nie ma połączenia, spróbuj je nawiązać
                    if not self.ntrip_sock:
                        self.connect_ntrip()
                    # Jeśli jest połączenie, wysyłaj GGA co określony interwał
                    elif time.time() - self.last_gga_send_time > self.NTRIP_GGA_INTERVAL:
                        try:
                            print(f"NTRIP_UPDATE: Wysyłanie GGA: {self.last_gga_for_ntrip.strip()}", end='\r', flush=True)
                            self.ntrip_sock.sendall(self.last_gga_for_ntrip.encode('ascii'))
                            self.last_gga_send_time = time.time()
                        except socket.error as e:
                            print(f"\nBłąd wysyłania GGA do NTRIP: {e}. Próba ponownego połączenia.")
                            self.ntrip_sock.close()
                            self.ntrip_sock = None
                ...
                print(f"\nBłąd parsowania NMEA: {e}")
                pass

    # --- Wątek: Odbieranie poprawek NTRIP i wysyłanie ich do GPS ---
    def ntrip_reader_thread(self):
        print("Uruchomiono wątek odbioru poprawek NTRIP.")
        while not self.stop_event.is_set():
            if not self.ntrip_sock:
                time.sleep(1) # Czekaj na nawiązanie połączenia przez inny wątek
                continue
            ...
            try:
                ...
                if data:
                    if self.ser and self.ser.is_open:
                        self.ser.write(data)
                        # print(f"Przekazano {len(data)} bajtów poprawek RTCM do GPS.", end='\r', flush=True)
                    else:
                        print("\nOdebrano dane NTRIP, ale port szeregowy jest zamknięty. Dane odrzucone.")
                else:
                    # recv zwrócił 0, serwer zamknął połączenie
                    print("\nPołączenie NTRIP zamknięte przez serwer. Próba ponownego połączenia.")
                    self.ntrip_sock.close()
                    self.ntrip_sock = None
            except socket.timeout:
                continue # To normalne, po prostu spróbuj ponownie
            except socket.error as e:
                print(f"\nBłąd gniazda w wątku NTRIP: {e}.")
                if self.ntrip_sock: self.ntrip_sock.close()
                self.ntrip_sock = None
            except serial.SerialException as e:
                print(f"\nBłąd zapisu do portu szeregowego (z wątku NTRIP): {e}")
                if self.ser: self.ser.close(); self.ser = None


    # --- Wątek: Wysyłanie danych przez Bluetooth ---
    def send_data_via_bluetooth_thread(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                self.bt_sock.sendall(data_to_send + b'\n') # Dodajemy newline jako separator
                # print(f"Wysłano: {data_to_send.decode()}", end='\r', flush=True)
            except socket.error:
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                status_map = {0: "Brak", 1: "FIX", 2: "FLOAT", 4: "RTK-FIX", 5: "RTK-FLOAT"}
                bt_status = "OK" if self.bt_sock else "Brak"
                ntrip_status = "OK" if self.ntrip_sock else "Brak"

                print(f"Lat: {lat:.6f}, Lon: {lon:.6f} | RTK: {status_map.get(status, f'Nieznany({status})')} | BT: {bt_status} | NTRIP: {ntrip_status}      ", end='\r')
                time.sleep(1)
            ...
            # Czekaj chwilę na zakończenie wątków
